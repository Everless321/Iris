...
echo "==> 启动 master"
rm -f data/iris.db*
./target/debug/iris-master > /tmp/zf-m.log 2>&1 &
MPID=$!
sleep 2
...
echo "==> 启动节点 c / b / a"
IRIS_NODE_ID=c IRIS_DATA_ADDR=0.0.0.0:7446 ./target/debug/iris-node > /tmp/zf-c.log 2>&1 & CPID=$!
IRIS_NODE_ID=b IRIS_DATA_ADDR=0.0.0.0:7445 ./target/debug/iris-node > /tmp/zf-b.log 2>&1 & BPID=$!
IRIS_NODE_ID=a IRIS_DATA_ADDR=0.0.0.0:7444 ./target/debug/iris-node > /tmp/zf-a.log 2>&1 & APID=$!
sleep 4
